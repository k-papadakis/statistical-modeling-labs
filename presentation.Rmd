...
We then fit a linear model using all the variables. No variable is significant even though \( R^2 \) is decent. This leads us to believe that some variables are redundant due to multicollinearity.
```{r}
mod <- lm(mpg ~ ., vehicles)
summary(mod)
```
...
From the Residuals vs Fitted, we see that the line is not far from horizontal, and that the spread around it is roughly homogenous. Thus, homoscedacity seems to hold. The QQ plot doesn't look great, so we might need to transform the predicted variable. From the Residuals vs Leverage plot, we see that the two most influential samples are 29 and 9.
...
We see that according to the \(\frac{2p}{n}\) criterion, samples 9, 29 and 31 can be considered influential.
```{r}
...
kable(steps_all[1:10, ], caption = "10 Best results according to AIC")
```
...
As we noted before, the qqplot of the regressand doesn't look great. We'll try to take the logarithm of it incase it follows a lognormal distribution.
```{r}
par(mfrow=c(2,2))
plot(best_mod1)
par(mfrow=c(1,1))
